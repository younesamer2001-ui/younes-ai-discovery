...
import {
  ArrowRight, Phone, Bot, Shield, Globe, Menu, X, Mic, Brain,
  Volume2, Calendar, MessageSquare, Database, Zap, CheckCircle2,
  ArrowUpRight, Clock, Lock, Server, Sparkles
} from 'lucide-react'
...
function Nav({ lang, setLang }: { lang: 'no'|'en'; setLang: (l: 'no'|'en') => void }) {
  ...
  const links = lang === 'no'
    ? [{ href: '/', label: 'Hjem' }, { href: '/mobilsvarer', label: 'Mobilsvarer' }, { href: '/hvordan-det-fungerer', label: 'Hvordan det fungerer' }, { href: '/om-oss', label: 'Om oss' }]
    : [{ href: '/', label: 'Home' }, { href: '/mobilsvarer', label: 'AI Answering' }, { href: '/hvordan-det-fungerer', label: 'How it works' }, { href: '/om-oss', label: 'About' }]
  ...
  return (
    <>
      <style>{`
        ...
        @media(max-width:768px){.hide-mob{display:none!important}.show-mob{display:flex!important}.grid-2{grid-template-columns:1fr!important}}
      `}</style>
